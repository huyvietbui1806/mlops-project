...
class FeedbackRequest(BaseModel):
    """Ground-truth label cho một transaction — dùng để tính performance metrics."""
    transaction_id: str
    actual_is_fraud: bool
    feedback_source: Optional[str] = "manual"  # "manual" | "chargeback" | "review"


class DriftResponse(BaseModel):
    """Kết quả drift check từ Evidently."""
    drift_detected: bool
    drift_score: float
...
class EvaluateResponse(BaseModel):
    """Kết quả performance evaluation."""
...
class AlertWebhookPayload(BaseModel):
    """Payload từ Alertmanager webhook."""
    model_config = ConfigDict(extra="allow")   # Alertmanager gửi nhiều fields
