...
> [!NOTE]
> **Không cần cài thêm infrastructure phức tạp**: Prometheus + Grafana + Alertmanager sẽ chạy qua `docker-compose` — tách biệt hoàn toàn với app.
...
1. Bạn muốn log lưu ở **local file** (JSONL) hay push lên một storage ngoài (S3, DB)?  
   → Plan mặc định: **local JSONL** (`logs/predictions.jsonl`), dễ đọc bởi Evidently.
...
## File Tree Sau Khi Implement

```
mlops-project/
├── src/api/
│   ├── inference.py          (modified)
│   ├── main.py               (modified — Prometheus + logging + new endpoints)
│   ├── schemas.py            (minor: thêm FeedbackRequest)
│   ├── logger.py             (NEW)
│   ├── metrics.py            (NEW)
│   ├── drift.py              (NEW)
│   ├── evaluate.py           (NEW)
│   └── retraining_trigger.py (NEW)
├── deployment/
...
2. Gửi vài requests đến `/predict` → kiểm tra log file sinh ra
3. Hit `GET /drift` → kiểm tra báo cáo Evidently xuất hiện
4. Truy cập `localhost:9090` → verify metric `fraud_prediction_total` tăng
5. Truy cập `localhost:3000` → verify Grafana dashboard load data
6. Simulate drift (gửi request với dữ liệu bất thường) → verify alert fire
